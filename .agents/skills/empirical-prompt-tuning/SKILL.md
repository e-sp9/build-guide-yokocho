...
- Codex 環境では `.agents/skills/<skill>/SKILL.md` を canonical として評価する。
- Claude Code 環境では `.claude/skills/<skill>/SKILL.md` を canonical として評価する。
- 両方があるリポジトリでは mirror 側との差分を確認し、差分がなければ canonical だけを評価対象にする。修正を適用したら mirror 側にも同じ内容を反映する。
- mirror 側に差分がある場合は、差分を drift として記録し、原則 canonical を評価対象にする。mirror のみに存在する内容が明らかに新しい仕様に見える場合は、勝手に破棄せずユーザー確認または差分の意図を報告する。
- 「このリポジトリ内のスキル」のように対象が広い場合は、まず対象一覧を出し、実行型 skill を empirical 評価の優先対象にする。ここでの実行型 skill は、調査・編集・生成・レビューなど外部作業を実行して成果物を返す skill を指す。評価手法そのものや自己参照になる skill は、empirical 評価の対象から外し、必要に応じて構造審査モードとして切り分ける。
...
2. **バイアス排除読み**: 指示を「白紙」の実行者に読ませる。新規実行者コンテキストを作れる dispatch primitive（Claude Code: Task / Agent、Codex: `spawn_agent`）で **新規 subagent を dispatch** する。自己再読で済ませない（直前に書いた文章を客観視することは構造的に不可能）。並列で複数シナリオを同時実行する場合は、同一ターン内で複数 subagent を起動する。dispatch 不能環境の扱いは「環境制約」節を参照。
...
4. **両面評価**: 戻ってきた結果から次を記録する。
   ...
   - **指示側の計測**（判定規則は本節で一元定義、他箇所は本節を参照する）:
     ...
     - ステップ数（dispatch tool の戻り値に usage メタの `tool_uses` 相当があればそのまま使う。Read / Grep も含める、除外しない。取得できない環境では `unavailable` と記録し、subagent 自己申告のツール使用概数を補助値として別欄に置く）
     - 所要時間（dispatch tool の usage メタの `duration_ms` 相当。取得できない環境では `unavailable`）
...
7. **収束判定**: 目安「連続 2 イテレーションで critical 全通過、新規の不明瞭点ゼロ、最低精度ライン（通常 90%）以上、かつ メトリクス改善が閾値以下（後述）」で停止。重要度が高いプロンプトは 3 連続にする。精度が低いまま停滞している場合は収束ではなく設計見直し候補として扱う。

## 評価軸

| 軸 | 取り方 | 意味 |
|---|---|---|
| 成功/失敗 | `[critical]` 項目が全て ○ か（二値） | 最低ライン |
| 精度 | 成果物が要件を何 % 満たしたか | 部分成功の程度 |
| ステップ数 | 実行者が使ったツール呼び出し / 判断ステップ数。usage metadata が取れない場合は `unavailable` | 指示の無駄遣いの指標 |
| 所要時間 | 実行者の duration_ms。usage metadata が取れない場合は `unavailable` | 認知負荷の代替指標 |
...
呼び出し側はレポートから自己申告部分を抽出し、`tool_uses` / `duration_ms` 相当を dispatch tool の usage メタから取得して評価軸表を埋める。usage メタが露出しない環境では `unavailable` と明記し、subagent の「ツール使用概数」は正式メトリクスではなく補助値として扱う。

## 環境制約

新規 subagent を dispatch できない環境（既に subagent として動作している、Task / Agent / `spawn_agent` 等が無効化されている等）では、本 skill は **適用しない**。
...
**構造審査モード**: empirical 評価ではなく、skill / プロンプトの **記述の整合性・明瞭性だけ** をチェックしたい場合は、構造審査モードとして明示的に切り分ける。これは上の「本 skill は適用しない」の例外ではなく、empirical 評価とは別の静的補助作業である。subagent への依頼プロンプトに「今回は構造審査モード: 実行ではなくテキスト整合性チェック」と明記する。これにより subagent は環境制約節の skip 動作に引っかからず、静的レビューを返せる。構造審査は empirical の代替ではなく補助（連続クリア判定には使えない）。

構造審査モードは dispatch 可能なら新規 subagent に読ませる。dispatch 不能環境では呼び出し元が静的審査を行ってよいが、empirical 評価としては数えず、最終報告に「structure review only」と明記する。

## 反復の打ち切り基準

- **収束（停止）**: 連続 2 回で次を **全て** 満たす:
  - `[critical]` 項目: 全シナリオで全て ○
  - 精度: 最低ライン（通常 90%、重要プロンプトは 95%）以上
  - 新規不明瞭点: 0 件
  - 精度の前回比改善: +3 ポイント以下（5% → 8% のような飽和）
  - ステップ数の前回比変動: ±10% 以内（`unavailable` の場合は判定から外し、その旨を書く）
  - duration の前回比変動: ±15% 以内（`unavailable` の場合は判定から外し、その旨を書く）
  - **過適合チェック**: baseline 準備時に hold-out 候補を 1 本予約しておき、収束判定時に初めて評価する。精度が直近平均から 15 ポイント以上落ちたら過適合。baseline シナリオ設計に戻って edge を足す。
- **発散（設計を疑う）**: 3 回以上イテレーションしても新規不明瞭点が減らない → プロンプトの設計方針自体が間違っている可能性。修正パッチで直すのをやめ、構造を書き直す
- **リソース打ち切り**: 重要度と改善コストが釣り合わなくなったら止める。打ち切る場合は、最大 iteration 数、最大 subagent 数、最大所要時間、または最低精度ライン未達のどれで止めたかを明示する。
...
- **イテレーションごとに変更多ぎ**: 「あのときの修正のどれが効いたか」が追えなくなる。1 修正 1 イテレーション
